...
def _config(maximum_concurrent_conversations: int = 2):
...
def _message_text(message_row: dict) -> str:
    return "".join(
        part["content"]
        for part in message_row.get("content", [])
...
def test_async_processing_of_different_conversations(monkeypatch):
    ...
    runtime = RuntimeContext(
        _config(maximum_concurrent_conversations=2),
        conn,
    )
    monkeypatch.setattr(
        mail,
        "send_reply_smtp",
        lambda outgoing, smtp_config: None,
    )
...
def test_fifo_processing_within_one_conversation(monkeypatch):
    ...
    second = mail.store_incoming_email(
        ...
        incoming=mail.IncomingEmail(
            ...
            in_reply_to=(
                f"<podhead.{first['thread_id']}.token@example.com>"
            ),
            timestamp=2,
        ),
        whitelist={
...
def test_failure_is_replied_to_without_persistent_retry(monkeypatch):
    ...
    assert history[1]["content"][0]["content"].startswith(
        "Traceback (most recent call last):\n"
    )
    assert history[1]["content"][0]["content"].endswith(
...
def test_fatal_error_sends_traceback_and_continues(monkeypatch):
    ...
    monkeypatch.setattr(
        mail,
        "send_reply_smtp",
        lambda outgoing, smtp_config: sent_bodies.append(
            outgoing.body
        ),
    )
    ...
    assert expected_traceback.startswith(
        "Traceback (most recent call last):\n"
    )
...
def test_error_response_smtp_failure_is_logged(
    monkeypatch,
    caplog,
):
    conn = _conn()
...
def test_assistant_sender_id_matches_conversation_owner(monkeypatch):
    ...
    async def run_test():
        await schedule_conversation(
            runtime,
            queued["thread_id"],
            lambda history, message_row: "done",
        )
        await asyncio.gather(*runtime.processing_tasks.values())
