...
def _config(*, workspace_path: str = "head_pod", process_root: Path | None = None):
...
def test_main_bootstraps_before_running_backend(monkeypatch):
    ...
    monkeypatch.setattr(
        main_module,
        "ensure_runtime",
        lambda config: calls.append(("ensure", config)),
    )
    monkeypatch.setattr(main_module, "run_backend", lambda config: sentinel)
    monkeypatch.setattr(
        main_module.asyncio,
        "run",
        lambda coro: calls.append(("run", coro)),
    )

    main_module.main()

    assert calls == [
        ("ensure", main_module.secrets),
        ("run", sentinel),
    ]
...
def test_resolve_workspace_host_path_relative_and_absolute(tmp_path):
    ...
    assert bootstrap.resolve_workspace_host_path(relative) == (
        bootstrap.REPO_ROOT / "head_pod"
    ).resolve()
...
def test_ensure_container_image_skips_rebuild_when_fingerprint_matches(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda name: {
            "Id": "image-1",
            "Labels": {bootstrap.IMAGE_FINGERPRINT_LABEL: "expected"},
        },
    )
    monkeypatch.setattr(
        subprocess,
        ...
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("image rebuild should be skipped")
        ),
    )

    assert bootstrap.ensure_container_image() == ("image-1", False)


def test_runtime_fingerprint_changes_only_for_container_build_inputs(
    monkeypatch,
    tmp_path,
):
...
def test_ensure_runtime_leaves_valid_running_container_unchanged(
    monkeypatch,
    tmp_path,
):
    calls = []
    ...
    monkeypatch.setattr(
        ...
        lambda config: calls.append("models"),
    )
    monkeypatch.setattr(
        bootstrap,
        "ensure_container_image",
        lambda: ("image-1", False),
    )
    monkeypatch.setattr(
        bootstrap,
        "_podman_exists",
        lambda kind, name: kind == "container",
    )
    monkeypatch.setattr(
        bootstrap,
        "_inspect_container",
        lambda name: _container_inspect(
            running=True,
            mount_source=expected_workspace,
        ),
    )
    monkeypatch.setattr(
        bootstrap,
        "_remove_container",
        lambda name: calls.append("remove"),
    )
    monkeypatch.setattr(
        bootstrap,
        "_create_container",
        lambda config, workspace: calls.append("create"),
    )
    monkeypatch.setattr(
        bootstrap,
        "_start_container",
        lambda name: calls.append("start"),
    )
    monkeypatch.setattr(
        bootstrap,
        "verify_container_environment",
        lambda config, expected_image_id=None: calls.append("verify"),
    )
    monkeypatch.setattr(
        bootstrap,
        "initialize_database",
...
def test_ensure_runtime_recreates_container_when_workspace_mount_is_wrong(
    monkeypatch,
    tmp_path,
):
    calls = []
    ...
    monkeypatch.setattr(bootstrap, "ensure_model_servers", lambda config: None)
    monkeypatch.setattr(
        bootstrap,
        "ensure_container_image",
        lambda: ("image-1", False),
    ...
    monkeypatch.setattr(
        bootstrap,
        "_create_container",
        lambda config, workspace: calls.append(("create", workspace)),
    )
    monkeypatch.setattr(
        bootstrap,
        "_start_container",
    ...
    monkeypatch.setattr(
        bootstrap,
        "verify_container_environment",
        lambda config, expected_image_id=None: None,
    )
    ...
    assert calls == [
        "remove",
        ("create", bootstrap.resolve_workspace_host_path(config)),
        "start",
    ]


def test_verify_container_environment_rejects_wrong_workspace_mount(
    monkeypatch,
    tmp_path,
):
    config = _config(workspace_path=str(tmp_path / "expected"))
    ...
    try:
        bootstrap.verify_container_environment(
            config,
...
def test_ensure_model_servers_skips_start_when_both_endpoints_are_healthy(
    monkeypatch,
    tmp_path,
):
    ...
    monkeypatch.setattr(
        bootstrap,
        "_start_model_server",
        lambda endpoint, label: (_ for _ in ()).throw(
            AssertionError("should not start healthy endpoint")
        ),
    )
...
def test_ensure_model_servers_starts_only_missing_endpoint(
    monkeypatch,
    tmp_path,
):
    config = _config(process_root=tmp_path)
    ...
    monkeypatch.setattr(
        bootstrap,
        "_is_endpoint_healthy",
        lambda endpoint: endpoint["port"] == 8080,
    )
    monkeypatch.setattr(
        bootstrap,
        "_start_model_server",
        lambda endpoint, label: started.append((label, endpoint["port"])),
    )
    monkeypatch.setattr(
        bootstrap,
        "_wait_for_endpoint",
        lambda endpoint, label: waited.append((label, endpoint["port"])),
    )
...
def test_ensure_model_servers_starts_both_missing_endpoints(
    monkeypatch,
    tmp_path,
):
    config = _config(process_root=tmp_path)
    ...
    monkeypatch.setattr(
        bootstrap,
        "_wait_for_endpoint",
        lambda endpoint, label: None,
    )

    bootstrap.ensure_model_servers(config)

    assert started == [
        ("Main model server", 8080),
        ("Embedding model server", 8081),
    ]


def test_ensure_model_servers_fails_when_binary_is_missing(
    monkeypatch,
    tmp_path,
):
    config = _config(process_root=tmp_path)
...
def test_ensure_model_servers_fails_when_model_file_is_missing(
    monkeypatch,
    tmp_path,
):
    config = _config(process_root=tmp_path)
...
def test_ensure_model_servers_does_not_start_duplicates(
    monkeypatch,
    tmp_path,
):
    config = _config(process_root=tmp_path)
