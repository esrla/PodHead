...
from backhead import db, secrets
...
MODEL_SERVER_STARTUP_TIMEOUT_SECONDS = 120.0
...
def validate_config(config=secrets) -> None:
    placeholder_values = {
    ...
    if any(value.startswith("replace-") for value in placeholder_values):
        raise ValueError(
            "Copy backhead/secrets.example.py to backhead/secrets.py "
            "and replace the demonstration values."
        )
    ...
    if not 0.0 <= config.SKILL_SIMILARITY_THRESHOLD <= 1.0:
        raise ValueError(
            "SKILL_SIMILARITY_THRESHOLD must be between 0.0 and 1.0, "
            f"got {config.SKILL_SIMILARITY_THRESHOLD}."
        )


def resolve_workspace_host_path(config=secrets) -> Path:
...
def ensure_podman_available() -> None:
    try:
        ...
        raise RuntimeError(
            "Podman is required to run PodHead. "
            "Install Podman and rerun `python -m backhead.main`."
        ) from exc
    ...
    if completed.returncode != 0:
        ...
        raise RuntimeError(
            "Podman is required to run PodHead. "
            "Install or fix Podman and rerun `python -m backhead.main`."
...
def _inspect(kind: str, name: str) -> dict:
    ...
    if completed.returncode != 0:
        ...
        raise PodmanVerificationError(
            completed.stderr.strip() or f"{label} {name!r} does not exist."
        )
...
def ensure_container_image() -> tuple[str, bool]:
    ...
    if not image_id:
        raise PodmanVerificationError(
            f"Image {CONTAINER_IMAGE_NAME!r} is missing an inspectable image id."
        )
...
def _workspace_mount_source(inspect: dict) -> Path | None:
    for mount in inspect.get("Mounts") or []:
        if Path(mount.get("Destination", "")) == Path("/workspace"):
...
def _has_forbidden_mounts(inspect: dict, workspace_host_path: Path) -> bool:
    forbidden_sources = {SECRETS_PATH, BACKEND_PATH, REPO_ROOT}
    ...
    for mount in inspect.get("Mounts") or []:
        ...
        if BACKEND_PATH in source.parents or SECRETS_PATH.parent in source.parents:
            return True
...
def _container_requires_recreation(
    ...
    return (
        inspect.get("Image") != expected_image_id
        or _workspace_mount_source(inspect) != workspace_host_path
        or _has_forbidden_mounts(inspect, workspace_host_path)
    )
...
def _create_container(config, workspace_host_path: Path) -> None:
    subprocess.run(
        [
            "podman",
            "create",
...
def _endpoint_health_url(endpoint: dict) -> str:
...
def _manages_local_process(endpoint: dict) -> bool:
    fields = (
        "executable_path",
        "model_path",
        "host",
    ...
    return any(endpoint.get(field) is not None for field in fields)


def _validate_managed_endpoint(
    endpoint: dict,
    *,
    label: str,
) -> tuple[Path, Path]:
    required_fields = {
        "executable_path": endpoint.get("executable_path"),
        "model_path": endpoint.get("model_path"),
        "host": endpoint.get("host"),
        "port": endpoint.get("port"),
        "context_size": endpoint.get("context_size"),
        "threads": endpoint.get("threads"),
    }
    ...
    if missing:
        raise RuntimeError(
            f"{label} is missing managed server configuration: "
            f"{', '.join(missing)}"
        )

    executable = _expanded_path(
        str(endpoint["executable_path"]),
        label=f"{label} llama-server executable",
    )

    if not executable.is_file():
        raise RuntimeError(
            f"{label} llama-server executable not found: {executable}"
        )

    model_path = _expanded_path(
        str(endpoint["model_path"]),
        label=f"{label} model file",
    )
...
def _build_model_server_command(
    endpoint: dict,
    *,
    label: str,
) -> list[str]:
    executable, model_path = _validate_managed_endpoint(
        endpoint,
        label=label,
    )
...
def _start_model_server(endpoint: dict, *, label: str) -> None:
    process = subprocess.Popen(
        _build_model_server_command(endpoint, label=label),
...
def _wait_for_endpoint(endpoint: dict, *, label: str) -> None:
    ...
    raise RuntimeError(
        f"{label} did not become healthy at "
        f"{_endpoint_health_url(endpoint)} after startup."
    )


def ensure_model_servers(config=secrets) -> None:
    managed_endpoints = (
    ...
    for label, tag, endpoint in managed_endpoints:
        if not _manages_local_process(endpoint):
            continue
...
def verify_container_environment(
    ...
    *,
    ...
    if not state.get("Running"):
        raise PodmanVerificationError(
            f"Container {config.PODMAN_CONTAINER_NAME!r} is not running."
        )

    if expected_image_id is not None and inspect.get("Image") != expected_image_id:
        raise PodmanVerificationError(
            f"Container {config.PODMAN_CONTAINER_NAME!r} "
            "is not using the expected image."
        )

    if _workspace_mount_source(inspect) != workspace_host_path:
        raise PodmanVerificationError(
            "Expected configured workspace to be mounted at /workspace."
        )

    if _has_forbidden_mounts(inspect, workspace_host_path):
        raise PodmanVerificationError(
            "Forbidden backend or configuration mount detected."
        )
    ...
    if completed.returncode != 0:
        raise PodmanVerificationError(
            "/workspace/AGENT.md is missing inside the container."
        )
...
def ensure_runtime(config=secrets) -> None:
    ...
    if _podman_exists("container", config.PODMAN_CONTAINER_NAME):
        ...
        if _container_requires_recreation(
            inspect,
            image_id,
            workspace_host_path,
        ):
